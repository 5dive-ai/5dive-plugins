#!/usr/bin/env bun
/**
 * Pairing CLI for the telegram-pi plugin.
 *
 * Usage:
 *   bun /path/to/telegram-pi/pair.ts [--token=<bot-token>] [--timeout=60]
 *
 * Without args:
 *   - Reads TELEGRAM_BOT_TOKEN from ~/.pi/channels/telegram/.env
 ...
 *
 * Pattern: CLI-initiated (server can't be a coordination point because the pi
 * relay is the agent's main process — running this while the agent is up would
 * collide on the getUpdates token).
 *
 * Refuses to run if the relay is already polling (PID_FILE present and alive) —
 * two consumers on one getUpdates token conflict (409). The user must stop their
 * pi agent session first, pair, then restart it.
 */
...
if (!TOKEN) {
  console.error(
    `telegram-pi pair: bot token required.\n` +
...
try {
  ...
  if (pid > 1) {
    process.kill(pid, 0)
    console.error(
      `telegram-pi pair: another poller is already running (pid ${pid}).\n` +
      `  stop your pi agent session first, then re-run this command.`,
    )
    process.exit(2)
  }
} catch {}
...
try {
  ...
  console.error(`telegram-pi pair: getMe failed — bad token?\n  ${err}`)
  process.exit(3)
}
...
const paired = await new Promise<{ user_id: string; chat_id: string; first_name?: string } | null>(resolve => {
  ...
  void bot.start({ drop_pending_updates: true }).catch(err => {
    ...
    if (is409) {
      console.error(
        `telegram-pi pair: 409 Conflict — another bot poller is using this token.\n` +
        `  stop your pi agent session (or any other process polling this token), then retry.`,
      )
    } else {
      console.error(`telegram-pi pair: bot.start failed: ${err}`)
    }
    resolve(null)
  })
})
