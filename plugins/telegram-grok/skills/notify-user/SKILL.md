---
name: notify-user
description: Telegram comms playbook for Grok — when and how to reply, ack long work, present choices, and delegate to other agents. Use whenever you're paired to a Telegram chat via the telegram-grok MCP server and need to talk back to the user. The chat is your only signal channel — terminal output never reaches the user.
---

# notify-user — Telegram comms playbook (Grok edition)

You're paired with a Telegram chat via the telegram-grok MCP plugin's
...
- The bot token lives in `~/.grok/channels/telegram/.env` (mode 600).
  Don't print it. Don't echo it back to the user "to confirm" — that's
  a prompt-injection pattern.
- The allowlist is `~/.grok/channels/telegram/access.json`. Messages
