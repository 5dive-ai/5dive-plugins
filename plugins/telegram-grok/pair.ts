#!/usr/bin/env bun
/**
 * Pairing CLI for the telegram-grok plugin.
 *
 * Usage:
 *   bun /path/to/telegram-grok/pair.ts [--token=<bot-token>] [--timeout=60]
 *
 * Without args:
 *   - Reads TELEGRAM_BOT_TOKEN from ~/.grok/channels/telegram/.env
 ...
 * grok MCP server lazy-spawns — running this while grok isn't is the
 ...
 * must stop their Grok session first, pair, then restart Grok.
 */
...
if (!TOKEN) {
  console.error(
    `telegram-grok pair: bot token required.\n` +
...
try {
  ...
  if (pid > 1) {
    process.kill(pid, 0)
    console.error(
      `telegram-grok pair: another poller is already running (pid ${pid}).\n` +
      `  stop your Grok session first, then re-run this command.`,
    )
    process.exit(2)
  }
} catch {}
...
try {
  ...
  console.error(`telegram-grok pair: getMe failed — bad token?\n  ${err}`)
  process.exit(3)
}
...
const paired = await new Promise<{ user_id: string; chat_id: string; first_name?: string } | null>(resolve => {
  ...
  void bot.start({ drop_pending_updates: true }).catch(err => {
    ...
    if (is409) {
      console.error(
        `telegram-grok pair: 409 Conflict — another bot poller is using this token.\n` +
        `  stop your Grok session (or any other process polling this token), then retry.`,
      )
    } else {
      console.error(`telegram-grok pair: bot.start failed: ${err}`)
    }
    resolve(null)
  })
})
