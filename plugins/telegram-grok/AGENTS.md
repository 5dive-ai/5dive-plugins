# telegram-grok MCP — guidance for the Grok agent

This MCP server bridges a Telegram bot to your Grok session. The user
...
- **`wait_for_message`** — blocking. Returns the next user message as a
  ...
  max `timeout_seconds` is 50 — Grok kills any MCP tool call that runs
  past its `tool_timeout_sec` (default 60s), so asking for longer drops
...
- The token lives in `~/.grok/channels/telegram/.env` (mode 600).
- The allowlist lives in `~/.grok/channels/telegram/access.json`.
