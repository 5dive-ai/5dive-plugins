 ...
 * that starts it: a compaction is a model call, and the pilot must not be able to
 * delay a turn or fail one. Every outcome — including every refusal — is a line in
...
function considerBoundary($: EngineInterface, s: Live, percent: number | undefined): void {
  ...
  if (d.go) {
    // Not awaited: `startCompaction` puts the call on the sink's own chain, so a
    // compaction can neither delay a turn nor fail one.
...
function startCompaction($: EngineInterface, s: Live, percent: number): void {
  compacting = true
  const at = Date.now()
  flush = flush
