#!/usr/bin/env bun
/**
 * Pairing CLI for the telegram-codex plugin.
 *
 * Usage:
 *   bun /path/to/telegram-codex/pair.ts [--token=<bot-token>] [--timeout=60]
 *
 * Without args:
 *   - Reads TELEGRAM_BOT_TOKEN from ~/.codex/channels/telegram/.env
 ...
 * codex MCP server lazy-spawns — running this while codex isn't is the
 ...
 * must stop their Codex session first, pair, then restart Codex.
 */
...
if (!TOKEN) {
  console.error(
    `telegram-codex pair: bot token required.\n` +
...
try {
  ...
  if (pid > 1) {
    process.kill(pid, 0)
    console.error(
      `telegram-codex pair: another poller is already running (pid ${pid}).\n` +
      `  stop your Codex session first, then re-run this command.`,
    )
    process.exit(2)
  }
} catch {}
...
try {
  ...
  console.error(`telegram-codex pair: getMe failed — bad token?\n  ${err}`)
  process.exit(3)
}
...
const paired = await new Promise<{ user_id: string; chat_id: string; first_name?: string } | null>(resolve => {
  ...
  void bot.start({ drop_pending_updates: true }).catch(err => {
    ...
    if (is409) {
      console.error(
        `telegram-codex pair: 409 Conflict — another bot poller is using this token.\n` +
        `  stop your Codex session (or any other process polling this token), then retry.`,
      )
    } else {
      console.error(`telegram-codex pair: bot.start failed: ${err}`)
    }
    resolve(null)
  })
})
