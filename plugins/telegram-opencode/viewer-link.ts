...
export type TelegramTextPayload = { text?: string; parse_mode?: string; entities?: TelegramEntity[]; link_preview_options?: Record<string, unknown> }
...
function alreadyCodeWrapped(text: string, index: number, length: number, html: boolean): boolean {
  if (html) return text.slice(Math.max(0, index - 6), index).toLowerCase() === '<code>' && text.slice(index + length, index + length + 7).toLowerCase() === '</code>'
...
export function protectTelegramViewerLinks(payload: TelegramTextPayload): boolean {
  ...
  if (!mode) {
    ...
    payload.entities = [...prior.filter(entity => !viewerRanges.some(range => entity.offset < range.offset + range.length && range.offset < entity.offset + entity.length)), ...viewerRanges]
    ...
    for (const { index, link } of [...matches].reverse()) {
      ...
      const open = html ? '<code>' : '`'; const close = html ? '</code>' : '`'
  ...
  if (!payload.text.includes('Do not paste it back here')) {
    const suffix = mode === 'markdownv2' ? 'Copy\\-paste this one\\-time link into your browser\\. Do not paste it back here\\.' : WARNING
