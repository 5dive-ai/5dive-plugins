...
export function protectTelegramViewerLinks(payload: TelegramTextPayload): boolean {
  ...
  if (!mode) {
    ...
    const viewerRanges = matches.map(({ index, link }) => ({
      type: 'code',
      offset: index,
      length: link.length,
    }))
    // Explicit code entities are the parse-mode-free equivalent of MarkdownV2
    // code spans. Drop only entities that overlap the credential, so Telegram
    // cannot still interpret the same bytes as a text_link.
    payload.entities = [
      ...prior.filter(entity => !viewerRanges.some(range =>
        entity.offset < range.offset + range.length
          && range.offset < entity.offset + entity.length,
      )),
      ...viewerRanges,
    ]
    ...
    for (const { index, link } of [...matches].reverse()) {
      ...
      payload.text = payload.text.slice(0, index) + open + link + close
        + payload.text.slice(index + link.length)
    }
  }
  ...
  if (!payload.text.includes('Do not paste it back here')) {
    const suffix = mode === 'markdownv2'
      ...
      : WARNING
    // The preview suppression is the safety boundary. Avoid turning a valid
    // maximum-size message into a Telegram 400 merely to add explanatory copy.
