...
import { silenceFile } from './paths'
import type { SilenceState } from './types'

export function loadSilence(): SilenceState {
  try {
    return JSON.parse(readFileSync(silenceFile(), 'utf8')) as SilenceState
  } catch {
    return {}
  }
}
...
export function saveSilence(state: SilenceState): void {
  const tmp = `${silenceFile()}.tmp.${process.pid}`
  try {
    writeFileSync(tmp, JSON.stringify(state), { mode: 0o600 })
    renameSync(tmp, silenceFile())
  } catch {
